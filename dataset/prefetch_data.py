...
class data_prefetcher():
    def __init__(self, loader, mean=None, std=None, is_cutout=False, cutout_length=16, two_crop=False):
        self.loader = iter(loader)
        self.stream = torch.cuda.Stream()
        # t = 2 if two_crop else 1
        if mean is None:
        ...
        if two_crop:
            self.mean = torch.cat([self.mean, self.mean], 1)
            self.std = torch.cat([self.std, self.std], 1)
...
def fast_collate(batch):
    ...
    if not isinstance(imgs[0], list):
        ...
        h = imgs[0][0].size[1]
        tensor = torch.zeros((len(imgs), 3, h, w), dtype=torch.uint8)
        tensor1 = torch.zeros((len(imgs), 3, h, w), dtype=torch.uint8)
        for i, img in enumerate(imgs):
            nump_array = np.asarray(img[0], dtype=np.uint8)
            if(nump_array.ndim < 3):
                nump_array = np.expand_dims(nump_array, axis=-1)
            nump_array = np.rollaxis(nump_array, 2)
            ...
            nump_array = np.asarray(img[1], dtype=np.uint8)
            if(nump_array.ndim < 3):
                nump_array = np.expand_dims(nump_array, axis=-1)
            nump_array = np.rollaxis(nump_array, 2)
        ...
        tensor = torch.cat([tensor, tensor1], dim=1)
