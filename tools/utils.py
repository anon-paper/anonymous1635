...
def count_parameters_in_MB(model):
    return np.sum(np.prod(v.size()) for name, v in model.named_parameters() if "fc" not in name)/1e6

# def count_active_parameters_in_MB(parameters):# if "aux" not in name
#     return np.sum(np.prod(v.size()) for v in parameters)/1e6

def count_active_parameters_in_MB(opt):# if "aux" not in name
    return np.sum(np.prod(v.size()) for v in opt.parameters())/1e6
...
def save_checkpoint(state, is_best, path='.', filename='checkpoint.pth.tar'):
    ...
    torch.save(state, filename)
    if is_best:
        bestname = os.path.join(path, 'model_best.pth.tar')
