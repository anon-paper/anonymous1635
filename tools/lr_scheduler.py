    ...
    def get_lr(self):
        ...
        for base_lr in self.base_lrs:
            if self.use_warmup and self.last_step < self.warmup_steps:
                ...
                # if self.last_step % 1e5 == 0: print(self.cycle_length)
                # if self.last_step % 1e5 == 0: print(peri_iter)
...
def get_lr_scheduler(config, optimizer, num_examples=None):
    ...
    if config.optim.lr_schedule == 'cosine':
        ...
        raise NotImplementedError
    elif config.optim.lr_schedule == 'multistep':
        scheduler = MultiStepLR(optimizer, config.optim.milestones)
    else:
        raise NotImplementedError
