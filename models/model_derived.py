...
class Block(nn.Module):
    ...
    def __init__(self, in_ch, block_ch, head_op, stack_ops, stride):
        ...
        modules = []
        head=(in_ch == block_ch)
        modules.append(OPS[head_op](in_ch, block_ch, stride, 
                                        affine=True, track_running_stats=True, head=head))

        for stack_op in stack_ops:
            modules.append(OPS[stack_op](block_ch, block_ch, 1, 
                                        affine=True, track_running_stats=True, head=False))
...
class MBV2_Net(nn.Module):
    def __init__(self, net_config, config=None):
        ...
        self._num_classes = config.data.n_classes
        ...
        self.fc = nn.Linear(last_dim, self._num_classes)
    ...
    def forward(self,x):
        ...
        logits = self.fc(out.view(out.size(0),-1))
...
class RES_Net(nn.Module):
    def __init__(self, net_config, config=None, num_classes=128):
        """
        net_config=[[in_ch, out_ch], head_op, [stack_ops], num_stack_layers, stride]
        """
        ...
        input_block = []
        if self.in_chs == 32:
            input_block.append(nn.Conv2d(in_channels=3, out_channels=self.in_chs, kernel_size=3, 
                        stride=2, padding=1, bias=False))
        elif self.in_chs == 64:
            input_block.append(nn.Conv2d(3, self.in_chs, kernel_size=7, stride=2, padding=3, bias=False))
            input_block.append(nn.MaxPool2d(kernel_size=3, stride=2, padding=1))

        input_block.append(nn.BatchNorm2d(self.in_chs))
        input_block.append(nn.ReLU6(inplace=True))
        ...
        if self.net_config[-1][1] == 'bottle_neck' or 'bneck' in self.net_config[-1][1]:
    ...
    def forward(self, x, get_feature=False):        
        ...
        if get_feature:
            ...
            return logits
        # return logits if not get_feature else logits, out